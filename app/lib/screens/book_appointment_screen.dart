...
class _BookAppointmentScreenState extends State<BookAppointmentScreen> {
    ...
    try {
      ...
      final appointmentResponse = await ApiService.post(
        '/api/appointments/book-guest',
        {
          ...
          'patient_phone': _mobileController.text.trim(),
          'doctor_id': _selectedDoctorId,
          'date': DateFormat('yyyy-MM-dd').format(_selectedDate),
          'time_slot': timeSlot,
          'reason': _placeController.text.trim().isNotEmpty ? _placeController.text.trim() : null,
        },
      );

      if (appointmentResponse.statusCode != 200 && appointmentResponse.statusCode != 201) {
        throw Exception('Failed to book appointment: ${appointmentResponse.body}');
      }
      ...
      final paymentAmount = PaymentService.calculateAppointmentFee(_selectedHospital?.id ?? 0);
      final paymentOrderResponse = await PaymentService.createGuestPaymentOrder(
  ...
  Future<void> _confirmPayment() async {
    if (_paymentSessionId == null) {
      Fluttertoast.showToast(
        msg: 'Payment session not created. Please try again.',
        backgroundColor: AppColors.errorColor,
      );
      return;
    }
    ...
    try {
      // For guest flow, we cannot verify without auth. Let webhook confirm.
      Fluttertoast.showToast(
        msg: 'Payment status will update automatically after completion.',
        backgroundColor: AppColors.infoColor,
      );
      setState(() {
        _isLoading = false;
      ...
      Fluttertoast.showToast(
        msg: 'Error confirming payment: ${e.toString()}',
        backgroundColor: AppColors.errorColor,
      );
    }
  }
                ...
                Container(
                  ...
                  child: const Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 10),
                      Text(
                        'Once payment is done and appointment is booked, there will be no refund of the booking amount.',
                        style: TextStyle(
                          fontSize: 14,
                          color: AppColors.textDark,
                        ),
